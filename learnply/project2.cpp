...
// public function for constructing user contour at a specific height
...
// public function for constructing evenly spaced contours
void ScalarTopology::calcSpacedContours(int N)
{
	if (scalar_min == scalar_max)
	{
		std::cout << "Scalar min and max are equal" << std::endl;
		return;
	}
...
// find minimum and maximum scalar values for quad mesh scalar field.
void ScalarTopology::findMinMax()
{
	scalar_min = INFINITY;
	scalar_max = -INFINITY;
	...
	{
		if (m_poly->vlist[i]->scalar < scalar_min)
			scalar_min = m_poly->vlist[i]->scalar;
		if (m_poly->vlist[i]->scalar > scalar_max)
			scalar_max = m_poly->vlist[i]->scalar;
	}
}

// get vertex position values and scalar values for a given quad
...
// Calculates critical points of the quad mesh scalar field
void ScalarTopology::calcCriticals()
{
	// clear any save critical points
	criticals.clear();
	saddle_map.clear();
	...
	for (int i = 0; i < m_poly->nquads; i++)
	{
		...
		// get the positions and scalar values at each vertex of the quad
		...
		if (f11 - f21 - f12 + f22 == 0) // dont divide by zero
			continue;
		double x0, y0, f00;
		x0 = (x2 * f11 - x1 * f21 - x2 * f12 + x1 * f22) / (f11 - f21 - f12 + f22);
		y0 = (y2 * f11 - y2 * f21 - y1 * f12 + y1 * f22) / (f11 - f21 - f12 + f22);

		// use bilinear interpolation to get scalar at (x0,y0). (Lecture_08 slide 33)
		f00 = ((x2 - x0) * (y2 - y0) * f11 + (x0 - x1) * (y2 - y0) * f21 +
			   (x2 - x0) * (y0 - y1) * f12 + (x0 - x1) * (y0 - y1) * f22) /
			  ((x2 - x1) * (y2 - y1));

		// If the coordinates are outside the quad, continue
		if (x0 < x1 || x0 > x2 || y0 < y1 || y0 > y2)
			continue;
		
		// Otherwise, create a new critical point
		CriticalPoint crit;
		...
		// The Hessian matrix has zeros on the diagonal, so the eigenvalues (given on Lecture_08 slide 30)
	...
	// then loop through all the vertices to check
	// for any local mins and maxes
	for (int i = 0; i < m_poly->nverts; i++)
	{
	...
	// calculate the contours at each critical point
	crit_contours.clear();
...
// builds a polyline contour at the given scalar input s
PolyLine ScalarTopology::calcContour(double s)
{
	...
	for (int i = 0; i < m_poly->nedges; i++)
	{
		...
		// if one end point is above s and one is below, we have a crossing
